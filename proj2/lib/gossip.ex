defmodule GossipServer do
    ...
    def handle_cast({:start, numNodes, topology}, state) do
        ...
        finishNum = 0
        receiveCount = 0
        peerListPair = Enum.map(1..numNodes, fn index -> GenServer.start_link(GossipActor, {self(), receiveCount}) end)
...
defmodule GossipActor do
    ...
    def handle_cast({:neighbor, neighborListPID}, state) do
        {bossid, receiveCount} = state
        {:noreply, {bossid, receiveCount, neighborListPID}}
    end

    def handle_cast({:rumor, rumor}, state) do
        # IO.puts(rumor)
        {bossid, receiveCount, neighborListPID} = state
    ...
    def handle_cast({:send, rumor}, state) do
        ...
        {:noreply,{bossid, receiveCount, newNeighborListPID}}
    end
